...
module {
  ...
  public class UsersMetadata(initSet: Types.UsersMetadataStableState) {
    ...
    public func queryGetUserMonotonicCreateGoalCount(forUniqueUserId: Text): Nat {
      switch (getUserMetadata(forUniqueUserId)) {
        case (#err(msg)) { Prelude.unreachable(); }; // maybe not ideal if won't show error 
        case (#ok(exists)) { exists.monotonicCreateGoalCount; };
      };
    };
 
    // updates "next" field for a specific user's goal creation count; called when a new goal is created
    public func incrementSetAndGetUserMonotonicCreateGoalCount(forUniqueUserId: Text): Nat { 
      assert(userMetadataExists(forUniqueUserId));
      switch (getUserMetadata(forUniqueUserId)) {
        case (#err(msg)) { Prelude.unreachable(); };
        case (#ok(exists)) { 
          let next = exists.monotonicCreateGoalCount + 1;
          idToUsersMap_.put(forUniqueUserId, {
            epochCreationTime = exists.epochCreationTime;
            epochLastUpdateTime = Time.now();
            monotonicCreateGoalCount = next;
      ...
      switch exists {
        ...
        case (?exists) {
          idToUsersMap_.put(forUniqueUserId, {
            epochCreationTime = exists.epochCreationTime;
            epochLastUpdateTime = Time.now();
            monotonicCreateGoalCount = exists.monotonicCreateGoalCount;
